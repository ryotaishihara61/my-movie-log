...
export default async function MovieDetailPage({ params }: Props) {
  const id = params.id;
  const [movie, myLog] = await Promise.all([ getMovieDetails(id), getMyLog(Number(id)), ]);

  if (!movie) { return <div className="p-4">映画が見つかりませんでした。</div>; }
    ...
    <main className="container mx-auto p-4">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        <div className="w-full md:w-1/3">
          {movie.poster_path ? ( <img src={`https://image.tmdb.org/t/p/w500${movie.poster_path}`} alt={movie.title} className="rounded-lg shadow-lg" /> ) : ( <div className="bg-gray-700 aspect-[2/3] w-full rounded-lg flex items-center justify-center"><p className="text-xs text-gray-400">画像なし</p></div> )}
        </div>
