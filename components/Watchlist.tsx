...
export default function Watchlist({ initialWatchlist }: Props) {
  ...
  const handleMarkAsWatched = async (movieId: number) => {
    ...
    if (error) {
      ...
      // 画面上のリストからは即座に消す（これはUI/UXのため）
      setMovies(movies.filter((movie) => movie.id !== movieId));
      // ▼▼▼▼▼ ページ全体をサーバー側で再取得するよう命令 ▼▼▼▼▼
      router.refresh();
    }
  };

  const handleDelete = async (movieId: number) => {
    if (!window.confirm("この映画をリストから完全に削除しますか？")) {
      return;
    }
        ...
        <div key={movie.id} className="relative group">
          ...
          <div className="absolute top-2 right-2 flex flex-col gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              ...
              className="bg-black bg-opacity-70 text-white text-xs rounded-full px-2 py-1"
            >
              観た！
            </button>
            <button
              onClick={() => handleDelete(movie.id)}
              className="bg-red-700 bg-opacity-80 text-white text-xs rounded-full px-2 py-1"
            >
              削除
            </button>
          </div>
